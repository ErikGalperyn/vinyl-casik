...
module.exports = {
  ...
  create: async (data) => {
    ...
    const { title, artist, year, coverUrl, musicUrl, lyricsLrc, note, ownerId, genre = 'other' } = data;
    
    const query = db._isPostgres
      ? `INSERT INTO vinyls (title, artist, year, coverUrl, musicUrl, lyricsLrc, note, genre, ownerId)
        VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
        RETURNING id, title, artist, year, coverUrl, musicUrl, lyricsLrc, note, genre, ownerId, created_at, updated_at`
      : `INSERT INTO vinyls (title, artist, year, coverUrl, musicUrl, lyricsLrc, note, genre, ownerId)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`;
    
    const stmt = db.prepare(query);
    const result = await stmt.get(title, artist, year, coverUrl || null, musicUrl || null, lyricsLrc || null, note || null, genre || 'other', ownerId);
  ...
  update: async (id, data) => {
    ...
    const { title, artist, year, coverUrl, musicUrl, lyricsLrc, note } = data;
    ...
    const query = db._isPostgres
      ? `UPDATE vinyls 
         SET title = COALESCE($1, title),
             ...
             lyricsLrc = COALESCE($6, lyricsLrc),
      ...
      : `UPDATE vinyls 
         SET title = COALESCE(?, title),
             ...
             lyricsLrc = COALESCE(?, lyricsLrc),
             note = COALESCE(?, note)
         WHERE id = ?`;
    
    const stmt = db.prepare(query);
    if (db._isPostgres) {
      await stmt.run(title, artist, year, coverUrl, musicUrl, lyricsLrc, note, id);
    } else {
      await stmt.run(title, artist, year, coverUrl, musicUrl, lyricsLrc, note, id);
    }
